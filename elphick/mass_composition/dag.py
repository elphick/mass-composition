...
if __name__ == '__main__':
    ...
    # Add a node with the feed operation
    ...
    dag.add_node('lump', DAG.output, ['split'])
    dag.add_node('fines', DAG.output, ['split'])

    # Fit the DAG with the MassComposition object
    dag.run({'feed_1': mc_sample,
             'feed_2': deepcopy(mc_sample).rename('sample_2')
             })

    dag.plot()

    # Create an MCNetwork object from the dag
    mcn: MCNetwork = MCNetwork.from_dag(dag)
    mcn.plot()
    plt.show()

    df: pd.DataFrame = mcn.report()

    fig = mcn.plot_network()
    fig.show()

    fig = mcn.table_plot(plot_type='network')
    fig.show()
